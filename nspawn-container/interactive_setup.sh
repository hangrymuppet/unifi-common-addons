...
# interactive_setup
# This script guides you through creating a custom container on UnifiOS 3.0+ using systemd-nspawn. It includes steps for configuring the container, ensuring persistence across firmware updates, and installing custom services such as Pi-Hole (Not Yet) or AdGuard Home. The process involves installing systemd-container and debootstrap, and using them to create a base Debian system in the /data/custom/machines directory.

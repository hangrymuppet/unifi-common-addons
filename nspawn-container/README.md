...
This guide shows you how to create your own container on UniFi OS 4.x and install custom services in it (such as Pi-Hole or AdGuard Home).

**Warning**: If you recently migrated (or are planning to migrate) to UniFi's [Zone Based Firewall](https://help.ui.com/hc/en-us/articles/115003173168-Zone-Based-Firewalls-in-UniFi) (available on Network Application >= 9), then clients may not be able to reach the container network. [Step 2A.3](#step-2a-configure-the-container-to-use-an-isolated-macvlan-network) provides further details.
...
The following commands are all performed on the UniFi router in SSH.
...
2. Next, we use debootstrap to create a directory called `debian-custom` with a base Debian system in `/data/custom/machines`.
    ...
    - The container folder will be 390MB after installation, but it can increase to 1GB+ after installing many services (storage management is up to you).
...
2. Next, we will create a `debian-custom.nspawn` file in `/etc/systemd/nspawn` to configure parameters for the container (such as network, bind mounts, etc). Here, I use Vim to create the file as a personal preference.
    ...
    - For a container that has access to all the host network interfaces and full capabilities to do anything to the system, here is an example nspawn configuration file. Note that it is important to set `Boot=on` so that systemd boots up inside the container.
    ...
    - For a more isolated container configured with a macvlan bridge, follow [Step 2A](#step-2a-configure-the-container-to-use-an-isolated-macvlan-network) below instead of running the container.
...
4. Now that the container is running, we should be able to open a shell or log in to it.
...
- Typing `machinectl login debian-custom` will give you a login prompt like a normal Linux system. In most cases, you can use `machinectl shell` to bypass the login for easier access. If you do use the login instead of the shell, you can exit the container by holding the `Ctrl` key and pressing the `]` key 3 times.

1. Now that you have access to your own container, you can install whatever services you want within it like a normal Linux system ([see examples below](#step-4-install-custom-services)). Make sure you ran `machinectl enable debian-custom` so the container starts on boot.
...
1. Download the [10-setup-network.sh](scripts/10-setup-network.sh) script to `/data/on_boot.d` and configure it with your VLAN and IPs for your container and gateway. This script will create a brX.mac interface as a gateway bridge for containers to communicate with.
    ...
    - Modify `VLAN` to an existing VLAN ID that you want your container to be on. The default is VLAN 5 and subnet 10.0.5.0/24, with the gateway configured at 10.0.5.1. Make sure this VLAN and network are created in UniFi first, with a unique subnet and IP address.
    ...
    - Modify `IPV4_IP` to your preferred container IP. The default is 10.0.5.3, but you can use whatever you want as long as it's on the same subnet as the gateway subnet `IPV4_GW`.
    - Also modify `IPV6_GW` and `IPV6_IP` if you need IPV6 support. Leave them empty to turn off IPv6 support.

2. Configure Firewall
    - If you are running a network application < 9, or not using Zone-Based-Firewall yet, then go to step `4`.
    ...
    - visit `Uniti Dashboard -> Settings -> Profiles -> Network Objects` (object names below are examples, you can type any name).
      - Create a new object `Devices/IPv4: Pi-Hole DNS Servers` and add your container IP address (`10.0.5.3` in this example)
      - Create a new object `Ports: DNS` and add the following ports: `53`, `853`, `5053`
    - visit your UniFi dashboard -> Nework Application -> Settings -> Security -> Firewall tab
      ...
      - Action `Allow`
      - Destination Zone `External` (that's correct, even tho IP is internal, however, ZBF works with UniFi networks, and our VLAN (5 in this example) is not technically a UniFi network, and rules from `Internal` zone do not apply to it).
      - Check `IP` -> `Object`,  then from the dropdown select the address network object we've created above (`Devices/IPv4: Pi-Hole DNS Servers`).
      - Check `Port` -> `Object` and from the dropdown select the port network object we've created above (`Ports: DNS`)
      - Press `Add Policy`
      - In `Zone Matrix`, filter policies by `Internal` (Source) and `External` (Destination) and make sure the new policy is above all block rules; if not, scroll down, press `Reorder`, and move the new rule up
...
When the firmware is updated, `/data` (which contains our container storage) and `/etc/systemd` (which contains our boot scripts) are preserved, but `/var` and `/usr` are deleted by the firmware update script. Any additional Debian packages installed on the host OS, such as systemd-container, are also deleted. This means we need to reinstall the systemd-container package and re-link our container to /var/lib/machines (for machinectl access) when the firmware is upgraded. This can be accomplished with a simple boot script that checks whether this package is installed on boot.
...
Services can be installed in the container like any Linux system. For Debian containers, you can use apt or other manual methods. Follow the Debian/Linux guide for the specific software you want to install.
...
2. I am getting security errors trying to run certain privileged commands in the container.
    ...
    - Edit your `.nspawn` config file and add `Capability=all` under the `[Exec]` section to unlock all security capabilities. You can also permit or restrict capabilities for enhanced security (see the [nspawn manpage](https://www.freedesktop.org/software/systemd/man/systemd.nspawn.html#Capability=) for more information).

3. Some programs complain of a missing /lib/modules folder and can't access kernel modules.

    - Edit your `.nspawn` config file and add `BindReadOnly=/lib/modules` to the `[Files]` section. This will bind-mount the /lib/modules folder from the host OS. You might also want to try unlocking all capabilities or specific ones (as in the question above) if you're still having issues with permissions when using certain modules.
